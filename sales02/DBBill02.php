...
eof;
  ...
  public function TotalAmount($startDate, $endDate, $CustomerID){
    // 請求書の合計額
    $sql = <<<eof
    SELECT sum(salesinfo.Quantity*goods.Price) FROM salesinfo INNER JOIN goods ON salesinfo.GoodsID = goods.GoodsID WHERE (salesinfo.SalesDate BETWEEN ? AND ?) AND salesinfo.CustomeID = ?
eof;
  ...
  private function getSalesinfo($startDate, $endDate, $CustomerID){
    $sql = <<<eof
    SELECT salesinfo.id,salesinfo.SalesDate,salesinfo.GoodsID,goods.GoodsName,goods.Price,salesinfo.Quantity,(goods.Price*salesinfo.Quantity) FROM salesinfo INNER JOIN goods ON salesinfo.GoodsID=goods.GoodsID WHERE salesinfo.SalesDate BETWEEN ? AND ? AND salesinfo.CustomerID=? ORDER BY salesinfo.SalesDate,salesinfo.id
eof;
      $array = array($startDate, $endDate, $CustomerID);
      $res = parent::executeSQL($sql, $array);
      return $res;
    ...
    public function SelectSalesinfo($startDate, $endDate, $CustomerID){
      // $fieldCount = 7;
      $tag = "<table>\n";
      $tag .= "<tr><th>ID</th><th>日付</th><th>顧客名</th><th>商品名</th><th>単価</th><th>数量</th><th></th><th></th></tr>\n";
      $res = $this->getSalesinfo($startDate, $endDate, $CunstomeID);
      foreach($rows = $res->fetchAll(PDO::FETCH_NUM) as $row){
        $tag .= "<tr>";
        // 次の行のcount関数の引数は$row[0]にすること
